...
const CountTicket = ({ id, cart, setOpen }: PropsCounter) => {
    ...
    const decrementAndDelete = () => {
        if (productAmount === 1 && cart) {
            setOpen?.(true);
            return;
        }
    ...
    return (
        <div className={styles.counterStyle}>
            <Button
                onClick={decrementAndDelete}
                className={
                    productAmount < 1
                        ? styles.disabled
                        : classNames(styles.active, styles.button)
                }
