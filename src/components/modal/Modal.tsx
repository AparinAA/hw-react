...
const Button = ({ action, fill, children }: PropsButton) => {
    return (
        <button
            className={classNames(
                styles.button,
                fill ? styles.buttonFill : styles.buttonEmpty
            )}
            type="submit"
            onClick={() => action()}
        >
...
function Modal({ id, setOpen }: PropsModal) {
        ...
        <div className={styles.modal}>
            ...
            <div className={styles.modalQuestion}>
                <div className={styles.titleModal}>
                    <div className={styles.titleTextModal}>Удаление билета</div>
                    <button onClick={cancelDelete}>{iconClose}</button>
                </div>
                <div className={styles.modalText}>
                    Вы уверены, что хотите удалить билет?
                </div>
                <div className={styles.modalButtons}>
                    <Button action={aproveDelete} fill={"fill"}>
                        Да
                    </Button>
                    <Button action={cancelDelete}>Нет</Button>
                </div>
            </div>
        </div>
    );
